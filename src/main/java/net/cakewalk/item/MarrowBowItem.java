...
public class MarrowBowItem extends RangedWeaponItem implements Vanishable {
  public MarrowBowItem(Settings settings) {
    super(settings);
  }
        ...
        if ((double) f >= 0.1D) {
          ...
          if (!world.isClient) {
            ...
            PersistentProjectileEntity persistentProjectileEntity = arrowItem.createArrow(world, itemStack,
                playerEntity);
            persistentProjectileEntity.setProperties(playerEntity, playerEntity.pitch, playerEntity.yaw, 0.0F, f * 3.0F,
                1.0F);
